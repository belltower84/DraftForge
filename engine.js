(function(root,factory){
  ...
  class DraftForgeEngine{
    ...
    interveningPickSequence(){const nx=this.nextMine();if(!this.slot||!nx||nx<=this.overall)return[];const seq=[];for(let n=this.overall;n<nx;n++){const team=this.teamForOverall(n);if(team!==this.slot)seq.push({overall:n,team,round:this.roundFor(n)})}return seq}
    ...
    marketSurvivalBaseline(p){const nx=this.nextMine();if(!nx||nx<=this.overall)return null;const gap=nx-this.overall,expectedTaken=Math.max(0,p.rank-this.overall),z=(expectedTaken-gap*.72)/7;return Math.max(.02,Math.min(.98,1/(1+Math.exp(-z))))}
